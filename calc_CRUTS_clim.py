#!/usr/bin/env python
"""
Program     : Calculate mean climatology fields of CRUTS data for upload to the IPCC Data Distribution Centre
Author      : Neil Massey
...
def calculate_climatological_mean(var, years):
    # get input and output file path
    input_filepath = get_input_directory() + "/" + var + "/cru_ts3.24.01.1901.2015." + var + ".dat.nc"
    output_path = get_output_directory() + "/clim/" + var + "/netcdf/"
    output_filepath = output_path + "/cru_ts3.24.01." + str(years[0]) + "." + str(years[1]) + "." + var + ".clim.nc"
    # check if path exists
    if not os.path.isdir(output_path):
        os.makedirs(output_path)
    # build the cdo command - subtract the 1986->2005 mean from the mean over the period years[0]->years[1]
    cmd = ["ymonmean", "-selyear,"+str(years[0])+"/"+str(years[1]), input_filepath, output_filepath]
    cdo(cmd)
    ...
    for v in var_list:
        for cp in clim_periods:
            for years in CRUTS_climatological_periods[cp]:
                calculate_climatological_mean(v, years)
